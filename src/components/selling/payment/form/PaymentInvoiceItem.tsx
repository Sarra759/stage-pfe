...
//bedelna houni
 const handleAmountPaidChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // Dinero pour validation visuelle
  const dineroPreview = dinero({
    amount: createDineroAmountFromFloatWithDynamicCurrency(
      inputValue,
  ...
  // MAIS on envoie une valeur propre
  const safeAmount = Number(inputValue.toFixed(3));

  onChange({
    ...invoiceEntry,
    amount: safeAmount,
    digitAfterComma: currency?.digitAfterComma || 3 
  });
};
  return (
